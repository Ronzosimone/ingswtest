...
$stmt = $conn->prepare("Select o.codice as Codice,v.targaVeicolo,v.marca,v.modello,v.versione,v.targaVeicolo,o.durata - o.mesiPagati as rimanenti ,(((o.tanFisso * (v.Prezzo - o.anticipo) * (o.durata/12)) /100)+ v.Prezzo)/ o.durata as Rata,o.tanFisso,v.Prezzo,o.tipo,((o.tanFisso * (v.Prezzo - o.anticipo) * (o.durata/12)) /100)+ v.Prezzo as prezzoRimborsare from Veicolo v inner join Operazione o on v.targaVeicolo = o.targaVeicolo where o.IDutente = ?");
...
while($row = $result->fetch_assoc())
{
  ...
  $Dati = $Dati.'<td class="pt-3-half" >'.round($row['Rata'], 2).'</td>';
  ...
  $Dati = $Dati.'<td class="pt-3-half" >'.$row['prezzoRimborsare'].'</td>';
  $Dati = $Dati.'</tr>';
}
...
if ($stmt = $conn->prepare("SELECT tipo,durata,canoneMensile,totDaFinanziare,tanMensile,TargaVeicolo FROM Operazione WHERE IDutente = ? ")) {
	...
	if($tipo == 'leasing'){
		...
		$totLeasing;
		for($x = 1; $x <= $durata; $x++){
			$var = rand($canoneMensile,$rataMensile*1.20);
			$totLeasing .= $var.",";
			$totSoldiLeasing += $var;
		}
		...
		$totLeasing;
		for($x = 1; $x <= $durata; $x++){
			$var = rand($rataMensile,$canoneMensile*1.20);
			$totLeasing .= $var.",";
			$totSoldiLeasing += $var;
		}
		$totLeasing = substr($totLeasing,0,-1);
